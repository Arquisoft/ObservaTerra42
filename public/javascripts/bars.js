...
function bars(pathJson, width) {
	...
	function drawBars(error, data) {
		...
		if (error) {
			...
			x.domain([ 0, despX + d3.max(data, function(d) {
				return d.obsValue;
			}) ]);
			...
			// que le corresponda
			var bar = grafico.selectAll("g").data(data).enter().append("g")
					.attr(
							"transform",
							function(d, i) {
								return "translate(" + 3 * despX + "," + i
										* alturaBarra + ")";
							});

			// Escribe el código del país
			bar.append("text").attr("class", "countryCode").attr("x", -5).attr(
					"y", alturaBarra / 2).attr("dy", ".35em").text(function(d) {
			...
			bar.append("text").attr("x", function(d) {
				return x(d.obsValue) + 2 * despX;
			}).attr("y", alturaBarra / 2).attr("dy", ".35em").text(function(d) {
				return d.obsValue.toFixed(2) + " (" + d.country.name + ")";
			});
		}
	}
}